  ...
  return (
    <main className="min-h-screen bg-neutral-900">
      ...
      <div className="relative h-64 overflow-hidden">
        {data?.user?.banner && data?.user?.discordId ? (
          <Image 
            src={`https://cdn.discordapp.com/banners/${data.user.discordId}/${data.user.banner}.${data.user.banner.startsWith('a_') ? 'gif' : 'png'}?size=1024`} 
            alt="Banner" 
            fill 
            ...
            {data?.presence?.currentPage && !data.presence.currentPage.startsWith('Watching') ? (
              data.presence.currentPage.startsWith('Profile:') && data.viewedProfile ? (
                <div className="text-lg font-semibold">
                  ...
                  <a 
                    href={`/u/${data.viewedProfile.uid}`}
                    className="text-brand-400 hover:text-brand-300 hover:underline cursor-pointer"
                  >
                    {data.viewedProfile.name}
                  </a>
                </div>
              ) : (
                <a 
                  href={`/${data.presence.currentPage.toLowerCase()}`}
                  className="text-lg font-semibold text-brand-400 hover:text-brand-300 hover:underline cursor-pointer"
                >
              ...
              <div key={c.id} className="bg-neutral-800/50 rounded-lg p-4 border border-neutral-700/50">
                <div className="flex items-start gap-3">
                  ...
                  <div className="relative">
                    ...
                    >
                      <Image 
                        src={c.author?.image || '/placeholder.png'} 
