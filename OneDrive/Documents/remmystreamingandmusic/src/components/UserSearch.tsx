...
export default function UserSearch({ adminData }: UserSearchProps) {
  const [searchType, setSearchType] = useState<'uid' | 'name' | 'email'>('uid')
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/admin/user-lookup?type=${searchType}&query=${encodeURIComponent(searchQuery.trim())}`)
      if (res.ok) {
    ...
    <div className="p-6 bg-neutral-900/50 border border-neutral-700/50 rounded-xl backdrop-blur-sm">
      ...
      <form onSubmit={handleSearch} className="mb-6 space-y-3">
        <div className="flex gap-2">
          <select
            value={searchType}
            onChange={(e) => setSearchType(e.target.value as 'uid' | 'name' | 'email')}
            className="rounded-md bg-neutral-800 border border-neutral-700 px-3 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
          >
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {(adminData.isOwner || adminData.isDeveloper) && (
              <button className="px-4 py-2 bg-red-600/20 hover:bg-red-600/30 text-red-400 rounded-md text-sm flex items-center gap-2 transition-colors">
