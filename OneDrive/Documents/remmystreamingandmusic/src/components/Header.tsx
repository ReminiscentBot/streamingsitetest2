          ...
          <div className="relative flex-shrink-0 min-w-[120px]" ref={dropdownRef}>
            ...
            >
              <div className="relative w-8 h-8 rounded-full overflow-hidden">
                <Image src={getUserAvatar(session.user?.image)} alt={session.user?.name || ''} width={32} height={32} className="object-cover" />
              </div>
              ...
              <div className="absolute right-0 top-full mt-2 w-56 bg-neutral-900/95 backdrop-blur-sm border border-neutral-700/50 rounded-xl shadow-2xl overflow-hidden">
                <div className="p-4 border-b border-neutral-800/50 flex items-center gap-3">
                  <div className="relative w-12 h-12 rounded-full overflow-hidden">
                    <Image src={getUserAvatar(session.user?.image)} alt={session.user?.name || ''} width={48} height={48} className="object-cover" />
                  </div>
                  <div>
                ...
                <div className="py-2">
                  ...
                  {(adminData?.isOwner || adminData?.isDeveloper) && (
                    <Link href="/control" className="flex items-center gap-3 px-4 py-3 text-white hover:bg-neutral-800/50 transition-colors" onClick={() => setDropdownOpen(false)}>
